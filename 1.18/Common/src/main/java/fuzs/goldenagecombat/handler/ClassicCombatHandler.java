...
public class ClassicCombatHandler {
    ...
    public static EventResult onPlaySoundAtPosition(Level level, Vec3 position, MutableValue<SoundEvent> sound, MutableValue<SoundSource> source, DefaultedFloat volume, DefaultedFloat pitch) {
        // disable combat update player attack sounds
        if (!GoldenAgeCombat.CONFIG.get(ServerConfig.class).canceledAttackSounds.contains(sound.get())) {
            return EventResult.PASS;
        }
