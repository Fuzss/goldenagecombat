...
public class ClientCooldownHandler {
    ...
    public static void onAfterInit(Minecraft minecraft, Screen screen, int screenWidth, int screenHeight, List<AbstractWidget> widgets, Consumer<AbstractWidget> addWidget, Consumer<AbstractWidget> removeWidget) {
        ...
        boolean removeAttackCooldown = GoldenAgeCombat.CONFIG.getHolder(ServerConfig.class).isAvailable() && GoldenAgeCombat.CONFIG.get(ServerConfig.class).removeAttackCooldown;
        Option.ATTACK_INDICATOR.setTooltip(getAttackIndicatorTooltip(removeAttackCooldown));
        if (removeAttackCooldown) {
    ...
    private static <T> Function<Minecraft, CycleButton.TooltipSupplier<T>> getAttackIndicatorTooltip(boolean removeAttackCooldown) {
        return minecraft -> {
            List<FormattedCharSequence> lines;
            if (removeAttackCooldown) {
                lines = minecraft.font.split(ATTACK_INDICATOR_TOOLTIP, 170);
            } else {
                lines = List.of();
            }
            return $ -> {
